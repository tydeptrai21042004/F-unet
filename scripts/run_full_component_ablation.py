#!/usr/bin/env python3
"""Run the permanent full component ablation end to end."""
...
try:
    from .component_ablation_spec import DEFAULT_MODELS_CSV, DEFAULT_SEEDS
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config-dir", default="configs/full_component_ablation")
    parser.add_argument(
        "--output-root",
        default="outputs_full_component_ablation/etis",
    )
...
def main() -> None:
    ...
    latex_table = tables / "full_component_ablation.tex"
    delta_table = tables / "full_component_ablation_deltas.csv"

    run([py, str(PROJECT_ROOT / "tools" / "audit_repository_cleanliness.py")])
    run([py, str(PROJECT_ROOT / "tools" / "audit_full_component_ablation.py"), "--config-dir", args.config_dir])
    ...
    if args.run_tests:
        run(
            [
                ...
                "tests/test_full_component_ablation.py",
    ...
    run(
        [
            py,
            str(PROJECT_ROOT / "scripts" / "report_full_component_ablation.py"),
            "--summary-path",
            str(test_summary),
            "--latex-path",
            str(latex_table),
    ...
    print("\nFull component ablation completed successfully.")
